  ...
  <div class="max-w-5xl mx-auto bg-white rounded-lg shadow p-8">
    ...
    <div v-for="area in [...new Set(criteriosModulo1.map(c => c.area))]" :key="area" class="mb-8">
      ...
      <div class="divide-y border rounded-md">
        <div v-for="criterio in criteriosModulo1.filter(c => c.area === area)" :key="criterio.id"
          ...
          <div class="flex space-x-6 w-1/3 justify-end">
            <label class="flex items-center space-x-1">
              <input type="radio" :name="'criterio_' + criterio.id" value="3"
                v-model="form.itemsModulo1[criteriosModulo1.findIndex(c => c.id === criterio.id)].respuesta" />
