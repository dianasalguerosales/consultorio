...
<template>
  <div class="max-w-5xl mx-auto bg-white rounded-lg shadow p-8">
    <h3 class="text-2xl font-bold text-[#2D2B5B] mb-6">
      Anamnesis - Módulo 1 (Evaluación del Desarrollo Infantil)
    </h3>

    <!-- Agrupamos por área -->
    <div v-for="area in [...new Set(criteriosModulo1.map(c => c.area))]" :key="area" class="mb-8">
      <h4 class="text-lg font-semibold text-blue-700 mb-3">{{ area }}</h4>

      <!-- Tabla compacta -->
      <div class="divide-y border rounded-md">
        <div v-for="(criterio, index) in criteriosModulo1.filter(c => c.area === area)" 
             :key="criterio.id" 
          ...
          <div class="flex space-x-6 w-1/3 justify-end">
            <label class="flex items-center space-x-1">
              <input type="radio" value="3" v-model="form.itemsModulo1[index].respuesta" />
              <span class="text-xs">Adecuado</span>
            </label>
            <label class="flex items-center space-x-1">
              <input type="radio" value="2" v-model="form.itemsModulo1[index].respuesta" />
              <span class="text-xs">En desarrollo</span>
            </label>
            <label class="flex items-center space-x-1">
              <input type="radio" value="1" v-model="form.itemsModulo1[index].respuesta" />
