...
const form = useForm({
  ...
  activo: props.especialidad?.activo ? true : false
})

function submit() {
  form.put(route('especialidades.update', props.especialidad.id), {
...
<template>
  <ModalBaseEditar @close="$emit('close')">
    <template #header>
      <h2 class="text-2xl font-bold text-[#2D2B5B]">Editar Especialidad</h2>
    </template>
