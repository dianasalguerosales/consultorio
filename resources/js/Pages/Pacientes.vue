...
<template>
    ...
    <!-- Modal Historial -->
    <div v-if="showHistorial" class="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg shadow-xl w-2/3 max-w-3xl h-5/6 overflow-y-auto relative">
        <!-- Header -->
        <div class="flex justify-between items-center border-b p-4">
        <h2 class="text-xl font-bold text-caine-verde">
            Historial de citas de {{ selectedPaciente?.nombre }}
        </h2>
        <button @click="closeHistorial" class="text-gray-500 hover:text-gray-700 text-2xl">&times;</button>
        </div>
        ...
        <table class="w-full border-collapse">
            ...
            <tbody>
            <tr v-for="cita in selectedPaciente?.citas || []" :key="cita.id" class="hover:bg-gray-50">
