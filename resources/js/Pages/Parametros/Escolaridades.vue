...
import EscolaridadModalVer from '@/Components/parametros/EscolaridadModalVer.vue'
import EscolaridadModalEditar from '@/Components/parametros/EscolaridadModalEditar.vue'
...
const selectedEscolaridad = ref(null)

function openVerModal(escolaridad) {
  selectedEscolaridad.value = escolaridad
  showVerModal.value = true
}
function closeVerModal() {
  showVerModal.value = false
  selectedEscolaridad.value = null
}

function openEditarModal(escolaridad) {
  selectedEscolaridad.value = escolaridad
...
<template>
  <div class="overflow-x-auto bg-white rounded-lg shadow-md">
    <table class="min-w-full border border-gray-200 text-md rounded-lg">
      ...
      <!-- Filas -->
      <tbody>
        <tr v-for="e in escolaridades" :key="e.id" class="border-t hover:bg-[#FAF9F7] transition">
          <!-- Nombre -->
          ...
          <td class="px-4 py-2 text-center">
            <div class="flex justify-center space-x-2">
              ...
              <!-- Eliminar -->
              <Link as="button" method="delete" :href="`/parametros/escolaridades/${e.id}`"
    ...
    <EscolaridadModalVer v-if="showVerModal" :escolaridad="selectedEscolaridad" @close="closeVerModal" />
    <EscolaridadModalEditar v-if="showEditarModal" :escolaridad="selectedEscolaridad" @close="closeEditarModal" />
  </div>
</template>