...
import EstadoSesionModalVer from '@/Components/parametros/EstadoSesionModalVer.vue'

defineProps({
  estadoSesiones: Array
})

const showVerModal = ref(false)
const showEditarModal = ref(false)
const selectedEstadoSesion = ref(null)

function openVerModal(estadoSesion) {
  selectedEstadoSesion.value = estadoSesion
  showVerModal.value = true
}
function closeVerModal() {
  showVerModal.value = false
  selectedEstadoSesion.value = null
}

function openEditarModal(estadoSesion) {
  selectedEstadoSesion.value = estadoSesion
...
<template>
  <div class="overflow-x-auto bg-white rounded-lg shadow-md">
    <table class="min-w-full border border-gray-200 text-md rounded-lg">
      ...
      <!-- Filas -->
      <tbody>
        <tr v-for="e in estadoSesiones" :key="e.id" class="border-t hover:bg-[#FAF9F7] transition">
          <!-- Nombre -->
          ...
          <td class="px-4 py-2 text-center">
            <div class="flex justify-center space-x-2">
              ...
              <!-- Eliminar -->
              <Link as="button" method="delete" :href="`/parametros/estado-sesiones/${e.id}`"
    ...
    <EstadoSesionModalVer v-if="showVerModal" :estadoSesion="selectedEstadoSesion" @close="closeVerModal" />
  </div>
</template>