...
function submit() {
    form.post('/especialidades', {
        onSuccess: () => form.reset() // limpia campo después de guardar
    })
}

function startEdit(especialidad) {
    editando.value = especialidad.id
    form.nombre = especialidad.nombre
}

function update(especialidad) {
    form.put(`/especialidades/${especialidad.id}`, {
        onSuccess: () => {
            editando.value = null
            form.reset()
        }
...
const especialidadesFiltradas = computed(() => {
    return props.especialidades.filter(e => {
        const texto = `${e.id} ${e.nombre}`.toLowerCase()
...
<template>
    <div class="bg-white rounded-lg shadow-md p-6 w-full">
        ...
        <div class="flex justify-between items-center mb-6">
            <div class="relative">
                <span
                    class="material-icons absolute left-2 top-1/2 transform -translate-y-1/2 text-gray-400">search</span>
                <input v-model="search" type="text" placeholder="Buscar..."
                    class="pl-8 pr-3 py-2 border rounded-md text-md focus:ring-2 focus:ring-[#53C6D3]" />
            </div>
            <form @submit.prevent="submit">
                <div class="flex gap-2">
                    <input v-model="form.nombre" type="text" placeholder="Nueva especialidad"
        ...
        <div class="overflow-x-auto">
            <table class="w-full border border-gray-200 text-md rounded-lg">
                ...
                <tbody>
                    <tr v-for="(especialidad, index) in especialidadesFiltradas" :key="especialidad.id"
                        class="border-t hover:bg-[#FAF9F7] transition">
                        <td class="px-4 py-2 font-medium text-[#2D2B5B]">{{ index + 1 }}</td>
                        <td class="px-4 py-2">
                            <template v-if="editando === especialidad.id">
                        ...
                        <td class="px-4 py-2 text-center">
                            <div class="flex justify-center space-x-2">
                                <template v-if="editando === especialidad.id">
                                    <button @click="update(especialidad)"
