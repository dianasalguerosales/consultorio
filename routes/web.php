...
Route::middleware('auth')->group(function () {
    ...
    Route::middleware(['permission:ver agenda'])->group(function () {
        ...
        Route::put('/reprogramaciones/{solicitud}', [SolicitudReprogramacionController::class, 'update'])
            ->middleware('permission:agendar citas')
            ->name('reprogramaciones.update');

        // Escribir la sesión es del que evalúa y atiende. El controlador
        // verifica además que sea quien atiende esa cita.
        Route::post('/agenda/{cita}/sesion', [SesionController::class, 'store'])
            ->middleware('permission:gestionar evaluaciones')
    ...
    Route::middleware(['permission:gestionar evaluaciones'])->group(function () {
    ...
    // Pacientes. Consultar y escribir son permisos distintos: el terapeuta y
    // el rol de pruebas entran a mirar, pero no crean ni editan.
    ...
    Route::middleware(['permission:ver pagos'])->group(function () {
        Route::get('/pagos', [PagoController::class, 'index'])->name('pagos.index');
    });

    Route::middleware(['permission:registrar pagos'])->group(function () {
    ...
    Route::middleware(['permission:autorizar pagos'])->group(function () {
    ...
    // Usuarios. Entrar a la pantalla y crear cuentas son permisos distintos:
    // el rol de pruebas la consulta, y el auxiliar crea las de los padres.
    Route::middleware(['permission:ver usuarios'])->group(function () {
        ...
        // Personas: consultarlas es 'ver personas'; crearlas y editarlas,
        // 'gestionar personas'.
        Route::middleware(['permission:ver personas'])->group(function () {
            ...
            Route::middleware(['permission:gestionar personas'])->group(function () {
            ...
            // Roles de una persona: se le asignan a su usuario ligado. El
            // controlador limita ademas que roles puede otorgar cada quien.
            Route::put('/personas/{tipo}/{id}/roles', [RolesController::class, 'update'])
                ->middleware('permission:asignar roles')
                ->name('personas.roles.update');
    ...
    // Expedientes, programas y parametros: cada uno con su permiso. Antes
    // colgaban del mismo grupo que usuarios, asi que el terapeuta no entraba al
    // expediente por no poder ver la pantalla de usuarios.
    Route::middleware(['auth'])->group(function () {

        Route::get('/expedientes', [ExpedienteController::class, 'index'])
            ->middleware('permission:ver expedientes')
            ->name('expedientes.index');

        Route::middleware(['permission:gestionar expedientes'])->group(function () {
            ...
            // La anamnesis es parte del expediente y se llena en el mismo paso.
        ...
        Route::get('/programas', [AsignacionProgramaController::class, 'index'])
            ->middleware('permission:ver programas')
            ->name('programas.index');

        Route::middleware(['permission:gestionar programas'])->group(function () {
        ...
        Route::post('/programas/{asignacion}/renovar', [AsignacionProgramaController::class, 'renovar'])
            ->middleware('permission:gestionar programas')
            ->name('programas.renovar');

        Route::get('/parametros', [ParametroController::class, 'index'])
            ->middleware('permission:ver parametros')
        ...
        Route::middleware(['permission:gestionar parametros'])->group(function () {
